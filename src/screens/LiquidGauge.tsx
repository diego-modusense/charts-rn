...
export const LiquidGauge = ({navigation}) => {
  //   const svgWidth = Dimensions.get('screen').width * 0.97;
  const [value, setValue] = useState(0);
  const svgWidth = Dimensions.get('screen').width - 32;
  const svgHeight = 400;
  const radius = 112.5;
  ...
  const textPixels = (GAUGE_CONFIG.config.fontSize * radius) / 2;
  ...
  const [wavePosition, setWavePosition] = useState(14);

  const updateWavePosition = () => {
    if (wavePosition === 14) {
      setWavePosition(87);
    } else {
      setWavePosition(prevState => prevState - 1);
    }
  };

  useEffect(() => {
    if (wavePosition <= 87) {
      setTimeout(() => setWavePosition(wavePosition + 1), 15);
    } else {
      setWavePosition(14);
    }
  }, [wavePosition]);
  ...
  const getWaveYAxis = () => {
    const full = 62;
    const empty = 290;
    const totalWaterLevel = empty - full;

    if (GAUGE_CONFIG.value <= 0) return 300;

    if (GAUGE_CONFIG.value >= 100) return full;

    const tankPercentage = (totalWaterLevel * GAUGE_CONFIG.value) / 100;

    return empty - tankPercentage;
  };
      ...
      <View style={styles.contentWrapper}>
        <Svg width={svgWidth} height={svgHeight} style={styles.svg}>
          <G>
            ...
            <TextSvg
              textAnchor={'middle'}
              fontSize={GAUGE_CONFIG.config.fontSize}
        ...
        <Text style={styles.label}>{`RESULT -> ${getWaveYAxis()}`}</Text>
        <Text style={styles.label}>Water level Chart example</Text>
      </View>
    </Container>
  );
};
